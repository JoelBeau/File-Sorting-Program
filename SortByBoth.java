 ...
 * SortByBoth.java
 * Dec 1, 2020 11:11:43 AM
 */
public class SortByBoth implements Sort {
	 ...
	 * this case. Also invokes the setyears()
	 * 
	 * @see {@link #setOrgPath()}
	 * @see {@link #setYears()}
	 * @see {@link #create()}
	...
	/**
	 * @implNote Creates folders for the years using the create Method, then creates
	...
	public void create() {
		...
		for (int i = 0; i < months.length; i++) {
			...
			for (int j = 0; j < months.length; j++) {
				...
				File f = new File(this.orgPath + "\\" + this.years[i] + "\\" + this.months[j]);
				f.mkdirs();
			}
	 ...
	 *           key) and the year and the month in which it was last modified
	 *           (HashMap value), concatenates them for the @see
	 *           {@link #move(HashMap)} and putting those values into a HashMap
	 * 
	...
	public HashMap<String, String> collect() {
		...
		HashMap<String, String> pathsNModified = new LinkedHashMap<>();
		...
		for (int i = 0; i < this.all.length; i++) {
			...
			int mIndex = Integer.parseInt(p.getLastModified().substring(0, 2));
			int yIndex = Integer.parseInt(p.getLastModified().substring(6, 10));

			p.setMonth(this.months[mIndex - 1]);
			p.setYear(this.years[yIndex - 2009]);

			pathsNModified.put(p.getPath().toString(), p.getYear() + "\\" + p.getMonth());

		}

		return pathsNModified;
	 ...
	 * 
	 * @param HashMap, file path of the picture (HashMap key) and the month and year
	 *                 in which it was last modified (HashMap value)
	 * 
	...
	@Override
	public void move(HashMap<String, String> pathsNModified) throws IOException {

		for (Entry<String, String> e : pathsNModified.entrySet()) {
	...
	// Gets the original path of files
